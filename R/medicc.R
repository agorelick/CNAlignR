##' medicc2_tree_and_heatmap
##' @export
medicc2_tree_and_heatmap <- function(tree_xml_file, cn_profile_file, groups=NULL, title=NA, bp_to_mb=F, vertical=F, ybreaksize=2, build='hg19') { 
    require(ggplot2)
    require(ggtree)
    ...
    if(vertical==T) { 
        ...
        p1 <- ggtree(tree,layout='rect') + geom_nodelab(angle=0, hjust=0, size=2.5, color='blue') + theme_tree2(legend.position='none') + labs(x='MEDICC2 Edit distance')
        if(!is.null(groups)) {
            groups[grep('Normal',label), label:='N']
            p1 <- p1 %<+% groups 
    ...
    if(vertical) {
        p2 <- ggplot(scna) + 
            ...
            geom_hline(yintercept=seq(0,length(sample_order)), color='#bfbfbf', linetype='dashed', linewidth=0.25) +
        ...
        p2 <- ggplot(scna) + 
            ...
            geom_hline(yintercept=seq(0,length(sample_order)), color='#bfbfbf', linetype='dashed', linewidth=0.25) +
            geom_vline(xintercept=c(0,chr22$global_end),linewidth=0.25,color='#bfbfbf') +
            geom_rect(data=scna[!is.na(type)], aes(xmin=global_seg_start,xmax=global_seg_end,ymin=sample_id_int-1, ymax=sample_id_int+0, fill=type)) + 
            geom_rect(data=scna[is_loh==T & !is.na(type)], aes(xmin=global_seg_start,xmax=global_seg_end,ymin=sample_id_int-1, ymax=sample_id_int+0), fill=NA, linewidth=0.35, color='black') + 
            #geom_rect(data=scna[is_loh==F & !is.na(type)], aes(xmin=global_seg_start,xmax=global_seg_end,ymin=sample_id_int-1, ymax=sample_id_int+0, fill=type)) + 
            #geom_rect(data=scna[is_loh==T & !is.na(type)], aes(xmin=global_seg_start,xmax=global_seg_end,ymin=sample_id_int-1, ymax=sample_id_int+0, fill=type), linewidth=0.35, color='black') + 
            scale_color_manual(values=allele_cols, name='Parental copy') + 
            scale_fill_manual(values=cols, na.value='white',name='Change from\nparent node') + 
            geom_segment(aes(x=global_seg_start, xend=global_seg_end, y=ypos, yend=ypos, color=allele), linewidth=0.5) +
            theme_fit(base_size=9) +
...
##' extract_medicc2_support_tree
##' @export
extract_medicc2_support_tree <- function(tree_xml_file, normal_sample) { 
    require(rphyloxml)
    xmltree <- read_phyloxml(tree_xml_file)
    tree <- phyloxml2phylo(xmltree)[[1]]
    ...
    node_dat <- rbindlist(l, fill=T)
    node_dat[isleaf==F, node_label:=name]
    node_dat[!is.na(confidence) & !is.na(node_label),node_label_confidence:=confidence]
    node_dat[is.na(confidence) & !is.na(name), node_label_confidence:=name]
    tree$node.label <- node_dat$node_label_confidence[node_dat$isleaf==F]
    tree$tip.label[tree$tip.label=='diploid'] <- normal_sample
    tree$node.label[!is.na(tree$node.label)] <- tree$node.label[!is.na(tree$node.label)]
    tree$node.label[grepl('internal',tree$node.label) | is.na(tree$node.label)] <- NA
